#!/bin/bash
# End-to-end tests for the seafile-traefik-letsencrypt-docker-compose backup + restore flow.
#
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-seafile}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-seafile-traefik-letsencrypt-docker-compose.yml}"
...
: "${BACKUP_INTERVAL:=24h}"
: "${SEAFILE_DB_USER:=seafiledbuser}"
: "${SEAFILE_DB_PASSWORD:?set in .env}"
...
test_restore_roundtrip() {
  ...
  echo "  restoring the baseline with ./seafile-restore-database.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./seafile-restore-database.sh "$(basename "$baseline")" \
...
test_data_restore_roundtrip() {
  ...
  echo "  restoring $(basename "$newest") with ./seafile-restore-application-data.sh"
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./seafile-restore-application-data.sh "$(basename "$newest")" \
    || { fail "the shipped restore script failed"; return 1; }
