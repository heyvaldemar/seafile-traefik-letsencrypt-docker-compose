...
BACKUP_PATH="/srv/seafile-application-data/backups/"
RESTORE_PATH="/seafile/"
BACKUP_PREFIX="seafile-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$SEAFILE_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$SEAFILE_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
